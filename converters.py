...
class PolkascanHarvesterService(BaseService):
    ...
    def process_metadata(self, spec_version, block_hash):
        ...
        runtime = Runtime.query(self.db_session).get(spec_version) # None
        ...
        if runtime:
            ...
            # runtime_version_data = {'apis': [['0xdf6acb689907609b', 3], ['0x37e397fc7c91f5e4', 1], ['0x40fe3ad401f8959a', 4], ['0xd2bc9897eed08f15', 2], ['0xf78b278be53f454c', 2], ['0xab3c0572291feb8b', 1], ['0xbc9d89904f5b923f', 1], ['0x582211f65bb14b89', 1], ['0x37c8bb1350a9a2a8', 1]], 'authoringVersion': 1, 'implName': 'halongbay', 'implVersion': 1, 'specName': 'halongbay', 'specVersion': 1, 'transactionVersion': 1}
            ...
            try:
                ...
                #         spec_version=runtime_type_data["spec_version"], # Integer
                #         type_string=runtime_type_data["type_string"], # String(255)
                #         decoder_class=runtime_type_data["decoder_class"], # String(255)
                #         is_primitive_core=runtime_type_data["is_primitive_core"], # Boolean | False
                #         is_primitive_runtime=runtime_type_data["is_primitive_runtime"] # Boolean | False
                #     )
    ...
    def add_block(self, block_hash):
        ...
        json_block = self.substrate.get_chain_block(block_hash) # 0x72caad462a1ea1bfc19356868f542a02590774aeff950a8e13205dc7aea741e7

        parent_hash = json_block['block']['header'].pop('parentHash') # 0xf80b28f6dbea7fa8b99a046fab13a7a38f4881f2af61c4cfea31036561a3f675
        block_id = json_block['block']['header'].pop('number') # 0x4cb4
        extrinsics_root = json_block['block']['header'].pop('extrinsicsRoot') # 0xa288fe111c7ef4f15451be05eb6beb71e2f8e2b25ee3f9949a26fd44e8ac0e2d
        state_root = json_block['block']['header'].pop('stateRoot') # 0x8a2154018e2d786aad6413e5812de0a73043b61eb31649dd45dd0ba0927efaab
        digest_logs = json_block['block']['header'].get('digest', {}).pop('logs', None) # ["0x046175746880365580bfd5f6ed0a319e216dd69caf4d355bcaef02b691200e81db94017eeb26", "0x0466726f6e8801e4985e731e915a90d55d28bedec190623670cabd903c347d6185dda4736f33d200"]

        # Convert block number to numeric
        if not block_id.isnumeric():
            block_id = int(block_id, 16) # 0x4cb4 -> 19636

        # ==== Get block runtime from Substrate ==================
        self.substrate.init_runtime(block_hash=block_hash)

        self.process_metadata(self.substrate.runtime_version, block_hash) # self.substrate.runtime_version = 1
        ...
        try:
            ...
            events_decoder = self.substrate.get_block_events(block_hash, self.metadata_store[parent_spec_version]) # has error
        ...
        for extrinsic in extrinsics_data:
            ...
            extrinsic_data = extrinsics_decoder.decode() # error
